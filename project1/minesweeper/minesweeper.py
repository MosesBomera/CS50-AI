...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        if cell not in self.moves_made:
            ...
            # Iterate through the neighbors and generate information
            neighbors = set()
            for i in range(cell[0] - 1, cell[0] + 2):
                for j in range(cell[1] - 1, cell[1] + 2):

                    # If it is this cell, skip it
                    if (i, j) == cell:
                        continue

                    # Check if cell is within bounds
                    if 0 <= i < self.height and 0 <= j < self.width:
                        # If a neighbor is a known mine, reduce mine count.
            ...
            for sentence in self.knowledge:
                ...
                if newsentence.cells.issubset(sentence.cells):
                        temp = self.subset(sentence, newsentence)
                        if temp not in self.knowledge:
                            self.knowledge.append(temp)
                            self.update_knowledge(temp)
                        continue

                if sentence.cells.issubset(newsentence.cells):
                        temp = self.subset(newsentence, sentence)
                        if temp not in self.knowledge:
                            self.knowledge.append(temp)
                            self.update_knowledge(temp)
            # Finally, infer from sentence itself.
            self.update_knowledge(newsentence)
    ...
    def update_knowledge(self, sentence):
        """
        Updates the knowledge base depending on inferences made from the given sentence.
        """
        # Make Inferences from this new information
    ...
    def subset(self, this, other):
        """
        Returns a new sentence from a subset condition.
        """
